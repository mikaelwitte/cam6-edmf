module clubb_intr
  ...
  !! MKW TODO: add any dependencies needed by edmf?
  implicit none
...
!  define physics buffer indicies here
  ...
  ! MKW TODO: add edmf variables here?
...
!-------------------------------------------------------------------------------
    ...
    ! MKW TODO: what (if any) EDMF fields need to be registered here?
...
!$OMP END PARALLEL
    ...
    ! MKW 2020-02-05. Should all these be ilev (i.e. nz+1)? Marcin had these as 'I' avgflag, I changed to 'A'
    ! ---------------------------------------------------------------------------- !
    ...
    if (l_stats) then
       ! MKW TODO: figure out what stats_init_clubb does
    ...
    if (history_clubb) then
       ...
       call add_default('THETAL',           1, ' ') ! MKW added 2020-02-21
    ...
    !  MKW TODO: do we need to set any EDMF fields here?
...
!-------------------------------------------------------------------------------
  ...
  ! ZM microphysics
   ...
   ! For EDMF outputs to outfld
  real(r8), dimension(pcols,pverp) :: edmf_dry_a_output, edmf_moist_a_output, &
            edmf_dry_w_output, edmf_moist_w_output, &
            edmf_dry_qt_output, edmf_moist_qt_output, &
            edmf_dry_thl_output, edmf_moist_thl_output, &
            edmf_dry_u_output, edmf_moist_u_output, &
            edmf_dry_v_output, edmf_moist_v_output, edmf_moist_qc_output, &
            edmf_thlflx_output, edmf_qtflx_output, &
            s_ae_output, s_aw_output, s_awthl_output, &
            s_awqt_output, s_awql_output, s_awqi_output, &
            s_awu_output, s_awv_output

  ! For EDMF output from advance_clubb_core
  real(r8), dimension(pverp+1-top_lev) :: edmf_dry_a, edmf_moist_a, &
 ...
 !  Copy the state to state1 array to use in this routine
   ...
   ! MKW TODO: any pointers to physics buffer need to be set HERE
   ...
   rtm(1:ncol,pverp)  = rtm(1:ncol,pver) ! MKW NOTE: top values of CLUBB arrays at pverp=nz+1 are set to k=nz
   ...
   edmf_dry_a_output(:,:) = 0._r8
   edmf_moist_a_output(:,:) = 0._r8
   edmf_dry_w_output(:,:) = 0._r8
   edmf_moist_w_output(:,:) = 0._r8
   edmf_dry_qt_output(:,:) = 0._r8
   edmf_moist_qt_output(:,:) = 0._r8
   edmf_dry_thl_output(:,:) = 0._r8
   edmf_moist_thl_output(:,:) = 0._r8
   edmf_dry_u_output(:,:) = 0._r8
   edmf_moist_u_output(:,:) = 0._r8
   edmf_dry_v_output(:,:) = 0._r8
   edmf_moist_v_output(:,:) = 0._r8
   edmf_moist_qc_output(:,:) = 0._r8
   edmf_thlflx_output(:,:) = 0._r8
   edmf_qtflx_output(:,:) = 0._r8
   s_ae_output(:,:) = 0._r8
   s_aw_output(:,:) = 0._r8
   ...
   do i=1,ncol   ! loop over columns
      ...
      if (do_expldiff) then ! MKW NOTE: by default, set to TRUE
        do k=1,nlev
      ...
      do t=1,nadv    ! do needed number of "sub" timesteps for each CAM step
         ...
         call advance_clubb_core_api &
            ...
            sclrm, &
            pblh(i), ustar2(i), & ! MKW TODO: check that this is how ustar2 is indexed
            edmf_dry_a, edmf_moist_a, &
            edmf_dry_w, edmf_moist_w, &
            edmf_dry_qt, edmf_moist_qt, &
            edmf_dry_thl, edmf_moist_thl, &
            edmf_dry_u, edmf_moist_u, &
      ...
      do k=1,nlev+1
          ...
          ! MKW 2020-02-10: convert EDMF outputs to pressure coordinates
          edmf_dry_a_output(i,pverp-k+1) = edmf_dry_a(k)
          edmf_moist_a_output(i,pverp-k+1) = edmf_moist_a(k)
          edmf_dry_w_output(i,pverp-k+1) = edmf_dry_w(k)
          edmf_moist_w_output(i,pverp-k+1) = edmf_moist_w(k)
          edmf_dry_qt_output(i,pverp-k+1) = edmf_dry_qt(k)
          edmf_moist_qt_output(i,pverp-k+1) = edmf_moist_qt(k)
          edmf_dry_thl_output(i,pverp-k+1) = edmf_dry_thl(k)
          edmf_moist_thl_output(i,pverp-k+1) = edmf_moist_thl(k)
          edmf_dry_u_output(i,pverp-k+1) = edmf_dry_u(k)
          edmf_moist_u_output(i,pverp-k+1) = edmf_moist_u(k)
          edmf_dry_v_output(i,pverp-k+1) = edmf_dry_v(k)
          edmf_moist_v_output(i,pverp-k+1) = edmf_moist_v(k)
          edmf_moist_qc_output(i,pverp-k+1) = edmf_moist_qc(k)
          edmf_thlflx_output(i,pverp-k+1) = edmf_thlflx(k)
          edmf_qtflx_output(i,pverp-k+1) = edmf_qtflx(k)
   ...
   ! ------------------------------------------------- !
   ! MKW TODO: add EDMF contributions to output variables here?
   ...
   do k=1,pver
      do i=1,ncol
         !  buoyancy flux
         ! MKW NOTE: no EDMF contribution to wpthvp yet
   ...
   do k=1,pverp
      do i=1,ncol
         ...
         edmf_thlflx_output(i,k) = edmf_thlflx_output(i,k)*rho(i,k)*cpair
         edmf_qtflx_output(i,k)  = edmf_qtflx_output(i,k)*rho(i,k)*latvap
      enddo
   enddo
   ...
   do k=1,pver-1
      do i=1,ncol
         ...
         shalcu(i,k) = 0._r8 ! MKW TODO: put MF cld frac here?
         ...
         concld(i,k) = min(cloud_frac(i,k)-alst(i,k)+deepcu(i,k),0.80_r8) ! MKW TODO: add EDMF contribution
      enddo
   enddo
   ...
   if (single_column) then
      if (trim(scm_clubb_iop_name) .eq. 'ATEX_48hr'       .or. &
          ...
          trim(scm_clubb_iop_name) .eq. 'ARM_CC') then
             ...
             deepcu(:,:) = 0.0_r8
             concld(:,:) = 0.0_r8 ! MKW TODO: don't set to 0 when using SCAM w/EDMF
   ...
   ! MKW TODO: add EDMF cloud frac here
   ...
   call outfld( 'edmf_DRY_A'    , edmf_dry_a_output,         pcols, lchnk )
   call outfld( 'edmf_MOIST_A'  , edmf_moist_a_output,       pcols, lchnk )
   call outfld( 'edmf_DRY_W'    , edmf_dry_w_output,         pcols, lchnk )
   call outfld( 'edmf_MOIST_W'  , edmf_moist_w_output,       pcols, lchnk )
   call outfld( 'edmf_DRY_QT'   , edmf_dry_qt_output,        pcols, lchnk )
   call outfld( 'edmf_MOIST_QT' , edmf_moist_qt_output,      pcols, lchnk )
   call outfld( 'edmf_DRY_THL'  , edmf_dry_thl_output,       pcols, lchnk )
   call outfld( 'edmf_MOIST_THL', edmf_moist_thl_output,     pcols, lchnk )
   call outfld( 'edmf_DRY_U'    , edmf_dry_u_output,         pcols, lchnk )
   call outfld( 'edmf_MOIST_U'  , edmf_moist_u_output,       pcols, lchnk )
   call outfld( 'edmf_DRY_V'    , edmf_dry_v_output,         pcols, lchnk )
   call outfld( 'edmf_MOIST_V'  , edmf_moist_v_output,       pcols, lchnk )
   call outfld( 'edmf_MOIST_QC' , edmf_moist_qc_output,      pcols, lchnk )
   call outfld( 'edmf_thlflx'   , edmf_thlflx_output,        pcols, lchnk )
   call outfld( 'edmf_qtflx'    , edmf_qtflx_output,         pcols, lchnk )
