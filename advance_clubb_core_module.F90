...
!-----------------------------------------------------------------------
  ...
  subroutine advance_clubb_core &
             ( l_implemented, dt, fcor, sfc_elevation, hydromet_dim, & ! intent(in)
               ...
               s_ae, s_aw, s_awthl, s_awqt, s_awql, s_awqi, s_awu, s_awv, & ! intent(inout)
               ! back to CLUBB stuff
#ifdef GFDL
    ...
    real( kind = core_rknd ), intent(inout) :: pblh, ustar
...
! <--- h1g, 2012-06-14
    ...
    !!! Local Variables
    integer :: i, k, nup, & ! MKW NOTE: nup is an EDMF variable, number of updraft plumes
#ifdef CLUBB_CAM
      ixind, &
#endif
...
! MKW TODO: update rcm and cloud_frac?
      ...
      call advance_xm_wpxp( dt, sigma_sqd_w, um, vm, wm_zm, wm_zt, wp2, & ! intent(in)
                            ...
                            tau_C6_zm, Skw_zm, rtpthvp, rtm_forc_mf,  & ! intent(in)
